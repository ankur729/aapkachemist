...
export class Cart1Component implements OnInit {
  ...
  cart: any = { 'cart_amount': '', 'delivery_amount': '', 'cart_count': '', 'gross_amount': '', 'items': [], 'customer_discount_wallet': '', 'lead_plan_detail_id': '', 'lead_plan_id': '', 'customer_discount_wallet_amount': '', 'net_payble_amount': '','prescription_status':0 };
  respdata: any = { 'categories_list': [] }
  // errtxt:any='';
  // iserr:boolean=false;
   ...
   // console.log(data);
    ...
    this.api.fetchCartData(data).subscribe(
      ...
      //    this.child.cartcount=dt.data.length;
          ...
          setTimeout(() => {
            this.child.onUpdateCartCountFromParent({'cartcount':dt.data.length});
            this.onResetCart(dt.data.length);
          }, 20);
          if (dt.data.length == 0) {
  ...
  removeItem(productdata) {
    ...
    if (this.state.is_logged_in) {
      ...
      data = "session_id=" + cdata.session_id + "&user_id=&rowid=" + cdata.rowid + "&user_id=" + this.state.user.id + "&direct_enquiry=" + direct_enquiry + "&vendor_id=" + this.state.vendor_id + "&removecart=" + cdata.removecart;
    }
    else if (!this.state.is_logged_in) {
      data = "session_id=" + cdata.session_id + "&user_id=&rowid=" + cdata.rowid + "&user_id=" + "&direct_enquiry=" + direct_enquiry + "&vendor_id=" + this.state.vendor_id + "&removecart=" + cdata.removecart;
    }

    this.api.removeCartItemApi(data).subscribe(
      (response) => {
        // alert('444');
      ...
      //  this.child.cartCountEvent();
        ...
        //  this.child.cartcount = dt.cart_count;
          ...
          this.child.onUpdateCartCountFromParent({'cartcount':dt.cart_count});
          this.onResetCart(dt.cart_count);
  ...
  onNext() {
    ...
    if (!this.state.is_logged_in) {
      ...
      this.router.navigate(['/login'], { queryParams: { redirecturl: '/cart-step-one' } });
    }
    else {
    ...
    //   console.log(this.state);
        if(state.cartdata.prescription_status==0){
          this.router.navigate(['/cart-step-two']);
        }
        else if(state.cartdata.prescription_status==1 || state.cartdata.prescription_status==2){
...
onResetCart(count){
  ...
  this.state.cartdata.cartcount=count;
