...
export class MyWalletComponent implements OnInit {
  ...
  state: any;
  wallet: any = { 'added_amt': '','lists':[],'balance':'0.0','is_pay_model_set':false ,'payment_history':{'payment_amount':'','txn_message':''},'payment_history_status':'0' }
...
// payu_model:any={'CUST_ID':'','FIRSTNAME':'','MOBILE_NO':'','EMAIL':'','type_id':'','app_type':'','TXN_AMOUNT':''}
    ...
    this.api.walletList(data).subscribe(
      (response) => {
        ...
        if (dt.status == 200) {
          ...
          //'payment_history':{'payment_amount':'','txn_message':''},'payment_history_status':'0' 
          if(dt.payment_history_status ==1){

            this.wallet.payment_history.payment_amount= dt.payment_history.payment_amount;
            this.wallet.payment_history.txn_message= dt.payment_history.txn_message;
            this.wallet.payment_history_status= dt.payment_history_status;
 ...
 //   let data ="CUST_ID="+this.state.user.id+"&INDUSTRY_TYPE_ID=Retail"+"&type_id=1"+
   ...
   let data ="CUST_ID="+this.state.user.id+
              ...
              "&wallet_type="+this.global.paytm.wallet_type_chemist;
    console.log(data);
    /// console.log(this.address);
