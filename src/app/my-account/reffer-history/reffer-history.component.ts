...
declare var $: any;
@Component({
  selector: 'app-reffer-history',
  templateUrl: './reffer-history.component.html',
  styleUrls: ['./reffer-history.component.css']
})
export class RefferHistoryComponent implements OnInit {
...
// payu_model:any={'CUST_ID':'','FIRSTNAME':'','MOBILE_NO':'','EMAIL':'','type_id':'','app_type':'','TXN_AMOUNT':''}
  ...
  getReferralHistory() {
    ...
    this.api.getReferralHistory(data).subscribe(
      (response) => {
       ...
       // this.global.setToast('info', dt.message);
        ...
        if (dt.status == 200) {
          ...
          if(this.wallet.lists.length==0){
            ...
            this.nodataobj.txt = "No history found";
             this.nodataobj.img_url = this.global.noimgfound.no_refferal_found;
          ...
          console.log(this.global.noimgfound.no_refferal_found);
          this.isnodatafound = true;
          this.nodataobj.page = 'enquiry';
          this.nodataobj.txt = "No history found";
           this.nodataobj.img_url = this.global.noimgfound.no_refferal_found;
