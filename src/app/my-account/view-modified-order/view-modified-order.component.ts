...
@Component({
  selector: 'app-view-modified-order',
  templateUrl: './view-modified-order.component.html',
  styleUrls: ['./view-modified-order.component.css']
})
export class ViewModifiedOrderComponent implements OnInit {
  ...
  respdata: any = { 'categories_list': [],'order_id':'','customer_id':'','enquiry_id':'','redirect_enquiry_id':'','vendor_id':'','direct_enquiry':''}
  ...
  //  console.log(this.state);
    ...
    this.respdata.vendor_id=   this.aroute.snapshot.queryParams["vid"];
  ...
  loadCartData() {
    ...
    let data:any;
      ...
      data = "order_id=" + this.respdata.order_id + "&user_id="+this.state.user.id + "&vendor_id=" +this.respdata.vendor_id+"&direct_enquiry="+this.respdata.direct_enquiry ;
  ...
  addToCartFn(productdata) {
    ...
    direct_enquiry+"&product_id=" + cdata.product_id + "&product_qty="+cdata.product_qty +"&product_mrp="+productdata.price;
     console.log(data);
  ...
  //  console.log(obj);
    if(obj.mode=='confirm'){
     ...
     //   this.router.navigate(['/my-account/order-detail'],{queryParams:{id:this.respdata.order_id,type:'Order'}})
        //my-account/order-detail?id=1689&type=Order
...
onCheckIfConfirmModifyOrder(){
  this.popup.onReceivePopupData({'type':'confirm','confirm_txt':'Are you sure accept this modification in your order','primary_btn_txt':'Confirm','secondary_btn_txt':'Close'});
...
onSubmitModifiedOrder(){
  ...
  data = "user_id="+this.state.user.id+"&vendor_id="+this.respdata.vendor_id+"&order_id="+this.respdata.order_id;
console.log(data);


this.api.onConfirMmodifyOrder(data).subscribe(
  (response) => {
    // alert('444');
  ...
  //  this.child.cartCountEvent();
    ...
    if (dt.status == 200) {
     ...
     // this.popup.onReceivePopupData({'type':'success','sent_txt':dt.message,'primary_btn_txt':'Send Quotation','secondary_btn_txt':''});
      this.popup.onReceivePopupData({'type':'success','sent_txt':dt.message,'primary_btn_txt':'View Order','secondary_btn_txt':''});
  //    this.child.onUpdateCartCountFromParent({'cartcount':dt.cart_count});
      // this.cart.cart_amount= this.calculateTotalMRP(this.state.cartdata.items);
      // this.cart.gross_amount =this.calculateGrandTotal(this.state.cartdata.items);
